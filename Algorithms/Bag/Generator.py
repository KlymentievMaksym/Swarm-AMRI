...
class Generator:
    def __init__(self, count: int, start_point: list[float, float], strength: float = 1, speed: float = 1, max_weight: float = 1, max_price: float = 100, min_amount: int = 20, items_per_square: int = 10, **kwargs):
        self.count = count
    ...
    def generate(self, **kwargs) -> np.ndarray:
        ...
        prices = self.dots[:, 2]
        random_elements = np.random.choice(prices, size=self.min_amount, replace=False)
        ...
        return self.dots[:, 2:], self.max_bag_weight
...
if __name__ == "__main__":
    # for i in range(100):
    gen = Generator(400, [5, 5], strength=1, speed=1, max_weight=10, max_price=10, items_per_square=10)
    gen.generate()
    gen.plot()
