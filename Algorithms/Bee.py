...
class BEE(Algorithm):
    def __init__(self, pop_size, iterations, areas_num, areas_elite_num, bees_depart_area_num, bees_depart_area_elite_num, random_limits, function, limits, **kwargs):
    ...
    def run(self, **kwargs):
        ...
        for iteration in range(self.iterations):
            ...
            new_best = self.fitness_func[np.argmin(self.fitness_func)]
            ...
            for l in range(self.areas_num):
                ...
                self.theta_max = np.random.uniform(self.low, self.high)
...
if __name__ == "__main__":
    ...
    #     return result
    # bee = BEE(100, 50, 20, 15, 10, 20, [.1, 1], F, [[-5.12, 5.12], [-5.12, 5.12]], d2=True, show=True, break_faster=True).run()
    ...
    # bee = BEE(100, 50, 50, 15, 8, 4, [0, 1], F, [[-1.5, 1.5], [-1.5, 1.5]], d2=True, show=True).run() # , save="Lab2/BEE.gif"
    ...
    # bee = BEE(100, 50, 50, 15, 8, 4, [0, 1], F, [[-10, 0], [-6.5, 0]], d2=True, show=True).run()
    ...
    # bee = BEE(100, 50, 50, 15, 8, [0, 1], F, [[-1.25, 1.25], [-1.25, 1.25]], d2=True, show=True).run()
    ...
    bee = BEE(100, 500, 20, 15, 10, 5, [0.1, 1], F, [[2.6, 3.6], [0.7, 0.8], [17, 28], [7.3, 8.3], [7.8, 8.3], [2.9, 3.9], [5.0, 5.5]], d2=False, show=True, integer=[2], break_faster=True)
    result = bee.run()
    print(*result)
    # print(F(result[1]))
    ...
    #     else:
    #         return float('inf')
    # bee = BEE(100, 50, 50, 15, 8, 4, [0, 1], F, [[0.005, 2.0], [0.25, 1.3], [2.0, 15.0]], d2=True, show=False, integer=[2])
    # result = bee.run()
    # print(*result)
    # print(F(result[1]))
