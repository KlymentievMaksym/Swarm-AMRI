    ...
    def run(self, **kwargs):
        ...
        for iteration in range(self.iterations):
            if self.random:
                ...
                self.frequency_high = np.random.uniform(self.low, self.high)
                if self.frequency_low > self.frequency_high:
                    self.frequency_high, self.frequency_low = self.frequency_low, self.frequency_high
...
if __name__ == "__main__":
    ...
    Bat(100, 100, .9, 1.9, .9, .5, .9, [.1, .3], [0, 1], Rastrigin, Rastrigin_limits*20, history=False, d2=True, show=True, random=True, fps=20).run()  #, save="Lab3/Images/Rastrigin.gif"
