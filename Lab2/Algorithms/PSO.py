    ...
    def run(self, **kwargs):
        ...
        for iteration in range(self.iterations):
            ...
            for i in range(self.pop_size):
                # low_un = 0.000000001
                a1 = np.random.uniform(0, 4)
                a2 = np.random.uniform(0, 4)
                ...
                # new_parts = np.maximum(self.x_low, np.minimum(self.x_high, self.parts[i]))
...
if __name__ == "__main__":
    ...
    pso = PSO(400, 70, [-1.1, 1.1], F, [[2.6, 3.6], [0.7, 0.8], [17, 28], [7.3, 8.3], [7.8, 8.3], [2.9, 3.9], [5.0, 5.5]], d2=False, show=True, integer=[2])
    result = pso.run()
    print(*result)
    print(F(result[1]))
    ...
    #     else:
    #         return float('inf')
    # pso = PSO(100, 50, [-1.1, 1.1], F, [[0.005, 2.0], [0.25, 1.3], [2.0, 15.0]], d2=True, show=False, integer=[2])
    # result = pso.run()
    # print(*result)
    # print(F(result[1]))
