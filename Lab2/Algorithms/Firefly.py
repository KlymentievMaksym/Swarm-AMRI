...
class FF(Algorithm):
    def __init__(self, pop_size, iterations, low_high, function, limits, **kwargs): # , beta_max, gamma, alpha
            ...
            for k in range(self.pop_size):
                for l in range(self.pop_size):
                    if self.function(self.parts[l]) < self.function(self.parts[k]):
                        ...
                        self.parts[k] = np.maximum(self.limits[:, 0], np.minimum(self.limits[:, 1], self.parts[k]))
...
if __name__ == "__main__":
    ...
    # ff = FF(50, 50, [0, 1], F, [[-1.5, 1.5], [-1.5, 1.5]], d2=True, show=True).run() # , save="Lab2/FF.gif"
    ...
    # for i in range(100):
    # ff = FF(400, 50, [0, 2], F, [[2.6, 3.6], [0.7, 0.8], [17, 28], [7.3, 8.3], [7.8, 8.3], [2.9, 3.9], [5.0, 5.5]], d2=True, show=False, integer=[2])
    # result = ff.run()
    # print(*result)
    # print(F(result[1]))
